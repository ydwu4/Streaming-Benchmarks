 ...
 */
public abstract class OnlineLearningModel {
    ...
    public OnlineLearningModel(String[] args) {
        ...
        paramSize = parameterTool.getInt("vector-size");
    ...
    public void modeling(StreamExecutionEnvironment env) {
        ...
        env.addSource(new FlinkKafkaConsumer010<String>( // source of samples
            ...
            new SimpleStringSchema(),
            parameterTool.getProperties()
        )).map(new MapFunction<String, LabeledVector>() { // convert each line to a sample
            public LabeledVector map(String s) throws Exception {
